#!/usr/bin/env python3
"""Convert visible G13 PNG images to the native 160x48 framebuffer layout.

The G13 has 160x43 visible pixels stored in six native vertical banks of eight
pixels. Rows 43 through 47 are padding and remain clear. Byte offset and bit
position:
...
G13_VISIBLE_WIDTH = 160
G13_VISIBLE_HEIGHT = 43
G13_NATIVE_HEIGHT = 48
G13_NATIVE_FRAME_BYTES = G13_VISIBLE_WIDTH * G13_NATIVE_HEIGHT // 8
G13_LAST_PAGE_VISIBLE_MASK = (
    1 << (G13_VISIBLE_HEIGHT % 8)
) - 1
G13_LAST_PAGE_PADDING_MASK = 0xFF ^ G13_LAST_PAGE_VISIBLE_MASK
...
class DecodedPng:
    ...
    @property
    def is_one_bit_grayscale(self) -> bool:
...
def monochrome_to_g13(
    ...
    width: int = G13_VISIBLE_WIDTH,
    height: int = G13_VISIBLE_HEIGHT,
) -> bytes:
    """Embed visible pixels in the zero-padded G13 vertical-byte layout."""

    if width != G13_VISIBLE_WIDTH or height not in (
        G13_VISIBLE_HEIGHT,
        G13_NATIVE_HEIGHT,
    ):
        raise ConversionError(
            f"G13 source must be exactly {G13_VISIBLE_WIDTH}x"
            f"{G13_VISIBLE_HEIGHT} visible pixels, or "
            f"{G13_VISIBLE_WIDTH}x{G13_NATIVE_HEIGHT} with empty padding; "
    ...
    if height == G13_NATIVE_HEIGHT:
        for y in range(G13_VISIBLE_HEIGHT, G13_NATIVE_HEIGHT):
            for x in range(width):
                if pixels[y * width + x]:
                    raise ConversionError(
                        f"padding pixel ({x},{y}) is set; rows "
                        f"{G13_VISIBLE_HEIGHT} through {G13_NATIVE_HEIGHT - 1} "
                        "must be empty in a 160x48 compatibility image"
                    )

    frame = bytearray(G13_NATIVE_FRAME_BYTES)
    for y in range(G13_VISIBLE_HEIGHT):
        for x in range(width):
            if pixels[y * width + x]:
                frame[x + (y // 8) * G13_VISIBLE_WIDTH] |= 1 << (y & 7)
    return bytes(frame)
...
def convert_png(
    ...
    image = read_png(path)
    if image.width != G13_VISIBLE_WIDTH or image.height not in (
        G13_VISIBLE_HEIGHT,
        G13_NATIVE_HEIGHT,
    ):
        raise ConversionError(
            f"{path} must be exactly {G13_VISIBLE_WIDTH}x"
            f"{G13_VISIBLE_HEIGHT} visible pixels, or "
            f"{G13_VISIBLE_WIDTH}x{G13_NATIVE_HEIGHT} with empty padding; "
...
def render_arrays(
    ...
    for index, frame in enumerate(frames, start=1):
        if len(frame) != G13_NATIVE_FRAME_BYTES:
            raise ConversionError(
                f"native frame {index} has {len(frame)} bytes; "
                f"expected {G13_NATIVE_FRAME_BYTES}"
            )
        if any(
            value & G13_LAST_PAGE_PADDING_MASK
            for value in frame[-G13_VISIBLE_WIDTH:]
        ):
            raise ConversionError(
                f"native frame {index} has non-zero padding bits in rows "
                f"{G13_VISIBLE_HEIGHT} through {G13_NATIVE_HEIGHT - 1}"
            )
    ...
    lines = [
        ...
        f"/* Visible: {G13_VISIBLE_WIDTH}x{G13_VISIBLE_HEIGHT}; native storage: "
        f"{G13_VISIBLE_WIDTH}x{G13_NATIVE_HEIGHT} "
        f"({G13_NATIVE_FRAME_BYTES} bytes). */",
        f"/* Native layout: offset=x+(y/8)*{G13_VISIBLE_WIDTH}, "
        "bit=(y&7), 1=white; padding rows are zero. */",
    ]
    ...
    if language == "c":
        lines.extend(
            [
                ...
                "#endif",
                "",
                f"#define {guard}_VISIBLE_WIDTH {G13_VISIBLE_WIDTH}",
                f"#define {guard}_VISIBLE_HEIGHT {G13_VISIBLE_HEIGHT}",
                f"#define {guard}_NATIVE_HEIGHT {G13_NATIVE_HEIGHT}",
                f"#define {guard}_FRAME_BYTES {G13_NATIVE_FRAME_BYTES}",
        ...
        lines.extend(
            [
                ...
                "#endif",
                "",
                f"static constexpr uint16_t {prefix}_visible_width = "
                f"{G13_VISIBLE_WIDTH};",
                f"static constexpr uint16_t {prefix}_visible_height = "
                f"{G13_VISIBLE_HEIGHT};",
                f"static constexpr uint16_t {prefix}_native_height = "
                f"{G13_NATIVE_HEIGHT};",
                f"static constexpr uint16_t {prefix}_frame_bytes = "
                f"{G13_NATIVE_FRAME_BYTES};",
    ...
    for identifier, frame in zip(identifiers, frames):
        lines.append(
            f"{declaration} {identifier}[{G13_NATIVE_FRAME_BYTES}] "
            "G13_FRAME_STORAGE = {"
        )
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert 160x43 visible PNG files to zero-padded native "
            "Logitech G13 arrays."
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        for source, (image, frame) in zip(sources, decoded_and_frames):
            pixel_mode = (
                "grayscale 1-bit"
                if image.is_one_bit_grayscale
                else f"converted color_type={image.color_type} bit_depth={image.bit_depth}"
            )
            geometry = (
                "visible source"
                if image.height == G13_VISIBLE_HEIGHT
                else "native-height compatibility source"
            )
            print(
                f"{source.name}: {image.width}x{image.height} {geometry}, "
                f"{pixel_mode}, "
